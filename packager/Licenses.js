  ...
  */
  static buildLicenses (electronOutputPath, npmLicenseString) {
    return new Promise((resolve, reject) => {
      ...
      fs.copySync(J('LICENSES.chromium.html'), J('vendor-licenses/LICENSES.chromium.html'), () => {
    ...
    return Promise.resolve()
      ...
      .then((npmLicenseString) => {
        return Promise.all(platforms.reduce((acc, platform) => {
          if (platform === 'darwin') {
            acc.push(Licenses.buildLicenses(path.join(DIST_PATH, 'openWMail-darwin-x64/'), npmLicenseString))
          } else if (platform === 'linux') {
            acc.push(Licenses.buildLicenses(path.join(DIST_PATH, 'openWMail-linux-ia32/'), npmLicenseString))
            acc.push(Licenses.buildLicenses(path.join(DIST_PATH, 'openWMail-linux-x64/'), npmLicenseString))
          } else if (platform === 'win32') {
            acc.push(Licenses.buildLicenses(path.join(DIST_PATH, 'openWMail-win32-ia32/'), npmLicenseString))
            acc.push(Licenses.buildLicenses(path.join(DIST_PATH, 'openWMail-win32-x64/'), npmLicenseString))
          }
          return acc
        }, []))
      })
